  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-[60vh] gap-6 animate-pulse">
        ...
        <p className="text-2xl font-black text-slate-300 uppercase tracking-tighter italic">Sincronizando Salidas...</p>
      </div>
    )
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-4">
        <div>
          <h1 className="text-7xl md:text-8xl font-black tracking-tighter text-slate-900 leading-[0.8] mb-4">
            Salidas<span className="text-rose-500">.</span>
          </h1>
          <p className="text-xl font-bold text-slate-400 uppercase tracking-widest italic ml-1">
            Egresos de caja y gastos rápidos
          </p>
        </div>
        <div className="bg-white px-10 py-6 rounded-[32px] shadow-sm border border-slate-50 flex flex-col items-end">
          <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1">GASTO ACUMULADO HOY</p>
          <h2 className="text-5xl font-black text-rose-500 tracking-tighter italic leading-none">{formatCurrency(getTotalExits())}</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <Card className="rounded-[44px] border-none shadow-sm bg-white overflow-hidden p-10 space-y-8 border border-slate-50">
          <div className="space-y-2">
            <div className="flex items-center gap-3 ml-1 mb-2">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">Importe de la salida</p>
            </div>
            
            {/* Massive Display */}
            <div className="bg-slate-50 rounded-[32px] p-8 md:p-10 border border-slate-100/50 group hover:border-rose-200 transition-colors duration-500 min-h-[160px] flex items-center">
              <div className="flex items-start justify-between w-full gap-4">
                <span className="text-4xl font-black text-rose-500/30 tracking-tighter mt-2 shrink-0">$</span>
                <div className="text-7xl md:text-8xl lg:text-9xl font-black text-rose-600 tracking-tighter italic leading-[0.8] break-all text-right flex-1">
                  {displayValue}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <Input
              placeholder="EJ. PAGO PROVEEDOR, GASTO LOCAL..."
              value={concept}
              onChange={(e) => setConcept(e.target.value)}
              className="h-20 px-8 rounded-[24px] border-none bg-slate-50 shadow-sm font-black text-xl uppercase tracking-widest placeholder:text-slate-200 focus-visible:ring-4 focus-visible:ring-rose-500/10 transition-all text-slate-900"
            />
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <Button
              ...
              className="h-20 col-span-2 rounded-[24px] bg-rose-500 hover:bg-rose-600 text-white font-black text-sm uppercase tracking-[0.2em] shadow-xl shadow-rose-500/20 active:scale-95 transition-all disabled:opacity-30"
            >
        ...
        <Card className="rounded-[44px] border-none shadow-sm bg-white overflow-hidden p-4 border border-slate-50">
          <div className="p-6 pb-2">
            <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 ml-4 mb-6">GASTOS DE HOY</h3>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {exits.length > 0 ? exits.map((exit) => (
                  <TableRow key={exit.id} className="group border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <TableCell className="px-8 py-6 font-black text-slate-400 text-[11px] font-mono">
                      {formatTime(exit.created_at)}
                    </TableCell>
                    <TableCell className="py-6">
                      <span className="text-sm font-black text-slate-900 uppercase tracking-tight italic">{exit.concept}</span>
                    </TableCell>
                    <TableCell className="px-8 py-6 text-right">
                      <span className="text-xl font-black text-rose-600 tracking-tighter italic">{formatCurrency(exit.amount)}</span>
                    </TableCell>
                  </TableRow>
                )) : (
                  <TableRow className="hover:bg-transparent">
                    <TableCell colSpan={3} className="h-64 text-center">
                      <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in duration-500">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sin gastos registrados hoy</p>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
